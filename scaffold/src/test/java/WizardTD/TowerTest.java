...
public class TowerTest extends App {
    ...
    public void testCorrectRange() {
        ...
        assertEquals(exp_range, test_tower.getRadius(), 0.001, "Incorrect tower radius was read from config file");
    }
    ...
    public void testCorrectSpeed() {
        ...
        assertEquals(exp_speed, test_tower.getSpeed(), 0.001, "Incorrect tower firing speed was read from config file");
    }
    ...
    public void testCorrectDamage() {
        ...
        assertEquals(exp_damage, test_tower.getDamage(), 0.001, "Incorrect tower damage was read from config file");
    }
    ...
    @Test
    public void testUnselectedSpeedUpgradeLevel() {
        Tower test_tower = new Tower(json, b.getTowerImages(), 20, 20, fireball);
        int exp_level = 0;
        test_tower.upgradeTower(mana,  u1, u3, u2);
    ...
    public void testFiringFireballs() {
        ...
        int x = (int)test_monster.getX()+32; //initial radius of tower range is 96
        int y = (int)test_monster.getY()+32;
    ...
    //test that tower firing speed increases after fastforward is selected
    @Test
    public void testFastForwardSelected() {
        int[] coords = board.getGrassTiles().get(3);
        Tower tower = new Tower(json, b.getTowerImages(), coords[0], coords[1], fireball);
        double exp_speed = tower.getSpeed()*2;
        ff.setSelected();
        tower.tick(this, b.getCostShow(), wave.getSpawnedMonsters(), mana, ff, p);

        assertEquals(exp_speed, tower.getSpeed(), 0.01, "Tower firing speed did not increase after fastforward was selected");
    }

    //test that tower firing speed reverts back to original speed after fast forward is unselected
    @Test
    public void testFastForwardUnselected() {
        int[] coords = board.getGrassTiles().get(3);
        Tower tower = new Tower(json, b.getTowerImages(), coords[0], coords[1], fireball);
        double exp_speed = tower.getSpeed();
        ff.setSelected();
        tower.tick(this, b.getCostShow(), wave.getSpawnedMonsters(), mana, ff, p);
        ff.setSelected();
        tower.tick(this, b.getCostShow(), wave.getSpawnedMonsters(), mana, ff, p);

        assertEquals(exp_speed, tower.getSpeed(), 0.01, "Tower firing speed did not revert after fastforward was selected");
    }
}
