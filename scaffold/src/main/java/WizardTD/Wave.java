...
public class Wave extends WaveElements{
    ...
    private int setFramesPerSpawn(double FPS) {
        double seconds = duration/total_monsters;
        int frames_per_spawn;
        if (FPS > 0) {
            frames_per_spawn = (int)(seconds*FPS);
        }
        else {
            frames_per_spawn = (int)(seconds*WaveElements.FPS);
        }
    ...
    public void tick(PApplet app, Pause pause, FastForward ff) { 
        if (current_wave_number == final_wave && total_monsters == 0) {
            ...
            int fps = FPS;
            if (ff.isSelected()) { //increase speed when fast forward is selected
            ...
            if ((int)pre_wave_pause > 0 && wave_framecount < fps*pre_wave_pause) {
                ...
                if (duration > 0) { //track duration of each wave
                    duration--;
                }
            ...
            if (next_pre_wave_pause > 0 || current_wave_number == final_wave) {
                ...
                for (Monsters current : spawned_monsters) {
                    ...
                    if (current.isAlive() && !current.getRespawn()) { //if monster is alive and hasn't reached wizard's house
                        current.draw(app); 
                        current.move();
                        current.respawnMonster(mana); //check if monster reaches wizards house
                    }
                    else if (!current.isAlive()) {   //if monster is dead
                    ...
                    else if (current.isAlive() && current.getRespawn() && frame_track == 0) { //if monster needs to be respawned and correct spawn rate
                        current.setRespawn();
                    }
                                        
                }
                for (Monsters remove: to_remove) { //remove dead monsters
                    spawned_monsters.remove(remove);
                }
            } 
            else if (duration == 0 && next_pre_wave_pause == 0) {
                if (current_wave_number < final_wave) { //new wave
                    current_wave_number++;
                    newWave(app);    
                }
            }
            ...
            for (Monsters current : spawned_monsters) { //if game is paused, don't move monster
                if (!current.disappeared()) {
                    current.draw(app);
                }
            }    
