...
public class BuildTower extends GameplayActions implements RetrieveValues{
    ...
    public BuildTower(Mana mana, JSONObject json, UpgradeRange range, UpgradeSpeed speed, UpgradeDamage damage, PImage tower_0, PImage tower_1, PImage tower_2, Board board, PImage ball) {
        ...
        tower_cost = (int)getNumericAmount("tower_cost", json);
    ...
    /**
     * Checks if the mouse has clicked a valid tile in Collection of x,y coordinates on the screen and sets the most recent coordinates to this.
     * Used to check if mouse has clicked a grass tile or a purchased tower tile
     * @param app that instantiates the class
     * @return true if mouse has clicked a valid tile, false otherwise
     */
    private boolean mouseClick(PApplet app, Collection<int[]> all_coords) {
        for (int[] coord : all_coords) {
            if (mouseClicked && (app.mouseX >= coord[0] && app.mouseX <= (coord[0] + RetrieveValues.PIXELSPERTILE)) && (app.mouseY >= coord[1] && app.mouseY <= (coord[1] + RetrieveValues.PIXELSPERTILE))) {
                this.coords = coord;
                return true;
            }
        }
